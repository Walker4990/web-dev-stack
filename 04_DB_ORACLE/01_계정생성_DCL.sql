...
-- CDB에 접속하면 공용 사용자를 만들 수 있는데, 계정 이름에 C## 접두사 필요
...
-- 사용자 계정 생성 구문 : 관리자 계정만 할 수 있는 역할
-- CREATE USER 계정명 IDENTIFIED BY 비밀번호;
CREATE USER KH IDENTIFIED BY KH;

/*
    DCL (DATA CONTROL LANGUAGE) : 데이터 제어 언어
    -계정한테 시스템 권한, 객체 접근 권한을
       부여(GRANT)하거나 회수(REVOKE)하는 구문
       GRANT 권한1, 권한2, ... TO 계정
       REVOKE 권한1, 권한2 ... FROM 계정
*/

/* CONNECT : 접속 권한 - 없으면 해당 유저로 접속 안됨.
   RESOURCE : 객체(생성, 수정, 삭제), 
              데이터(입력, 수정, 조회, 삭제) 권한 부여
*/
GRANT CONNECT, RESOURCE TO KH;
-- TABLESPACE : 오라클의 데이터를 저장하는 논리적 공간 단위
-- GRANT UNLIMITED TABELSPACE TO 계정명;
