/*
    ...
    - DB관리자, 설계자가 사용
    오라클에서 제공하는 객체(구조) / 스키마 : TABLE, VIEW, SEQUENCE, INDEX, PACKAGE, 
...
/*
    ...
    * 자료형
    1. 문자 : CHAR, VARCHAR2  - 반드시 크기 지정해야 함.
        - CHAR : 최대 2000 BYTE까지 저장 가능
                 고정 길이 (아무리 작은 값이 들어와도 처음 할당된 크기 그대로)
                 고정된 글자수의 데이터만 담길 때 사용
        - VARCHAR : 최대 4000 BYTE까지 저장 가능
                    가변 길이 (담긴 값에 따라서 공간의 크기를 맞춰 줌)
...
--                     모든 컬럼의 전반적인 구조를 확인 할 수 있는 시스템 테이블
...
/*
    ...
    COMMENT ON COLUMN 테이블명.컬렴명 IS '주석내용';
*/
COMMENT ON COLUMN MEMBER.MEM_ID IS '회원번호';

-- 테이블에 데이터추가 시키는 구문 (DML : INSERT)
...
/*
    ...
    - PRIMARY KEY 제약조건을 부여하면 그 컬럼은 자동으로 NOT NULL, UNIQUE 제약 조건 설정됨
...
INSERT INTO MEM_PK VALUES(1, 'user01', 'pass01', '윤의진', 
'남', '010-1111-2222', 'aaa@naver.com', '25/06/05');
INSERT INTO MEM_PK(MEM_NO, MEM_ID, MEM_PWD, MEM_NAME, MEM_DATE, GENDER) VALUES(2, 'user02', 'pass02', '이진용', SYSDATE, '남');
INSERT INTO MEM_PK(MEM_NO, MEM_ID, MEM_PWD, MEM_NAME, GENDER, MEM_DATE, EMAIL) 
            VALUES (1, 'user03', 'pass03', '곽병현', '남', SYSDATE, 'BBB@GOOGLE.COM');

SELECT * FROM MEM_PK;

-- PRIMARY KEY는 자동! SEQUENCE 사용 (ORACLE)
CREATE SEQUENCE MEM_SEQ START WITH 1 INCREMENT BY 1 NOCACHE NOCYCLE;

INSERT INTO MEM_PK VALUES(MEM_SEQ.NEXTVAL, 'user01', 'pass01', '윤의진', 
'남', '010-1111-2222', 'aaa@naver.com', '25/06/05');
INSERT INTO MEM_PK(MEM_SEQ.NEXTVAL, MEM_ID, MEM_PWD, MEM_NAME, MEM_DATE, GENDER) VALUES(2, 'user02', 'pass02', '이진용', SYSDATE, '남');
INSERT INTO MEM_PK(MEM_SEQ.NEXTVAL, MEM_ID, MEM_PWD, MEM_NAME, GENDER, MEM_DATE, EMAIL) 
            VALUES (1, 'user03', 'pass03', '곽병현', '남', SYSDATE, 'BBB@GOOGLE.COM');
...
INSERT INTO MEM_PK2(MEM_ID, MEM_PWD, MEM_NAME, GENDER, PHONE, EMAIL, MEM_DATE)
         VALUES('user01', 'pass01', '윤의진', '남', '010-1111-2222', 'aaa@naver.com', '25/06/05');
INSERT INTO MEM_PK2(MEM_ID, MEM_PWD, MEM_NAME, MEM_DATE, GENDER) VALUES('user02', 'pass02', '이진용', SYSDATE, '남');
INSERT INTO MEM_PK2(MEM_ID, MEM_PWD, MEM_NAME, GENDER, MEM_DATE, EMAIL) 
...
CREATE TABLE MEM_MEMBER2(
    ...
    MEM_PWD VARCHAR2(20) NOT NULL,
    MEM_NAME VARCHAR2(20) NOT NULL,
    GRADE_ID NUMBER REFERENCES MEM_GRADE(GRADE_CODE) --외래 키
);
...
/*
    ...
    * 삭제옵션 : 부모테이블의 데이터 삭제 시 그 데이터를 사용하고 있는 
    ...
    - ON DELETE SET NULL
     : 부모 데이터 삭제시 해당 데이터를 쓰고 있는 자식 데이터의 값을 NULL로 변경
    - ON DELETE CASCADE
...
/*
    ...
    - 컬럼을 선정하지 않고 INSERT 시 NULL이 아닌 
      기본 값을 INSERT 하고자 할 때 세팅해둘 수 있는 값
*/
...
CREATE TABLE MEMBER(
    ...
    HOBBY VARCHAR2(20) DEFAULT 'GAME',
...
INSERT INTO PUBLISHER(PUB_NAME, PHONE) VALUES ('오픈도어북스','010-0000-1111');
INSERT INTO PUBLISHER(PUB_NAME, PHONE) VALUES ('모티브','010-0000-2222');
SELECT * FROM PUBLISHER;
-- 테이블 명 : BOOK
-- 컬럼 :BK_NO - 숫자, 기본키
        -- BK_TITLE - 문자(50), NOT NULL
        -- BK_AUTHOR - 문자(50), NOT NULL
        -- PUB_NO - 숫자, 외래키(PUBLISHER 의 PUB_NO, 삭제 조건 CASCADE)
CREATE TABLE BOOK(
...
VALUES ('누구도 대신 살아주지 않는다.', '브라이언 트레이시', 1);
INSERT INTO BOOK (BK_TITLE, BK_AUTHOR, PUB_NO) 
...
INSERT INTO MEMBER(MEMBER_ID) VALUES('BEAR');
...
FROM 
    RENT A JOIN MEMBER B ON (A.MEMBER_NO = B.MEMBER_NO)
...
ALTER TABLE DEPT_COPY RENAME CONSTRAINT SYS_C008401 TO DEPT_ID_PK;

/*
    3. 테이블명 변경
    ALTER TABALE 기존 테이블명 RENAME TO 변경할 테이블명;
...
-- 참조되고 있는 부모테이블은 함부로 삭제되지 않는다.
DROP TABLE BOOK;

-- 만약 삭제하고자 한다면
-- 방법 1. 부모 테이블 삭제 시 제약조건까지 같이삭제
