/*
    ...
    - 기본 값(DEFAULT)이 지정되어 있으면 기본값이 들어감
...
INSERT ALL
    ...
    -- 테이블명의 컬럼이름을 기재안한 이유는 값에 전체 값이 다 들어가기때문에 안들어가도 된다.
    INTO EMP_MANAGER VALUES (EMP_ID, EMP_NAME, MANAGER_ID)
    SELECT EMP_ID, EMP_NAME, DEPT_CODE, HIRE_DATE, MANAGER_ID
    FROM EMPLOYEE
    WHERE DEPT_CODE = 'D1';

-- EMPLOYEE 에서 입사일 기준으로 2000년 1월 1일 이전 입사한 사원과
-- 이후 입사한 사원의 테이블을 분리
...
SET 
    MBTI = 'ENTJ'
WHERE MBTI = 'ISFP';
-- EMPLOYEE에서 EMP_NAME이 선동일인 사람의 급여(SALARY)를 7000원, 보너스 0.2로 변경
UPDATE EMPLOYEE
SET 
    SALARY = 7000,
    BONUS = 0.2
...
ROLLBACK;
-- MBTI가 ISFJ인 사람들 삭제
DELETE FROM USER_02 WHERE MBTI = 'ISFJ';

-- USER_02 사람들 전체 삭제
DELETE FROM USER_02;

--TRUNCATE를 사용해서 사람들 전체삭제 -> ROLLBACK 적용 X
TRUNCATE TABLE USER_02;
ROLLBACK;
SELECT * FROM USER_02;
