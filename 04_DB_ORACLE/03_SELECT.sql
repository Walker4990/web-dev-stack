/*
   ...
   - DBMS : DATABASE MANAGEMENT SYSTEM의 약자, DB관리 시스템
   - 관계형(RELATIONAL) 데이터베이스 : ORACLE, MYSQL
   1. 가장 대표적인 데이터베이스 시스템
   2. 데이터를 테이블 형태 (행과 열)로 저장하고, 여러 테이블 간 관계를 설정하고
        데이터의 관계를 표현 및 분석
   ...
   1. DML(Data Manipulation Language) : 데이터 조작어
      - 데이터를 조작하기 위한 언어
        UPDATE(수정), INSERT(추가), DELETE(삭제), SELECT(조회)
        C : CREATE - 추가 (INSERT)
        R : READ - 조회 (SELECT) - SELECT만 그냥 조회만함.
        U : UPDATE - 수정
        D : DELETE - 삭제
       - SELECT는 데이터 조회를 의미하여 일부에서는 DQL(Data Query Language)이라고 함.
   ...
   4. TCL(Transanction Control Language) : 트랜젝션 제어어
      - 트랜잭션 처리 및 제어를 위한 언어
...
*/
-- USER_INFO 에서 나이(AGE)의 10년 후를 계산
SELECT
    NAME 이름, AGE 나이, AGE+10 AS "10년 후" FROM USER_INFO;

-- EMPLOYEE에서 직원명(EMP_NAME), 월급(SALARY), 연봉 조회
SELECT
    EMP_NAME 이름, SALARY 급여, SALARY*12 AS 연봉
FROM EMPLOYEE;

/*
    리터럴(문자열)
    - SELECT 절에 리터럴 사용하면 테이블에 존재하는 데이터 처럼 조회 가능
    - 즉, 리터럴을 RESULT SET의 모든 행에 반복적으로 출력
    
    연결 연산자 : || 
    - 여러 컬럼 값들을 마치 하나의 컬럼인 것처럼 연결
    - 컬럼 값과 리터럴도 연결할 수 있음
...
SELECT 
    NAME ||'의 MBTI는'||  MBTI || '입니다.' MBTI 
FROM
    USER_INFO;

/*
    DISTINCT
    - 컬럼에 중복된 값들을 한 번씩만 표시
    - 주의사항 : DISTINCT는 SELECT절에서 한 번만 기술 가능
    
*/
-- USER_INFO에서 나이(AGE)가 중복 되지 않게 조회
...
/*
    ...
    FROM 테이블명
    WHERE 조건식
    ORDER BY 정렬하고자 하는 컬럼 값
    [ASC : 오름차순 (DEFAULT), DESC : 내림차순]
    실행 순서 : FROM - WHERE - GROUP BY - HAVING - SELECT - ORDER BY
*/

-- USER_INFO에서 나이를 오름차순으로 정렬
SELECT *
FROM USER_INFO
ORDER BY AGE;
...
--ORDER BY 4, 2 DESC;
--컬럼 순서를 숫자로 표현 할 수 있다.
-- NULLS LAST : ORACLE에서만 사용 가능, NULL 마지막으로 보냄.
-- NULLS FIRST : ORACLE에서만 사용 가능, NULL 맨 앞으로 보냄.

-- EMPLOYEE에서 전체 사원의 사원명, 보너스 조회
-- 보너스 기준 오름차순 정렬
SELECT EMP_NAME, BONUS
FROM EMPLOYEE
--ORDER BY BONUS NULLS FIRST -- 오름차순인 경우는 NULL이 맨 뒤에 옴.
ORDER BY BONUS DESC NULLS LAST; -- 내림차순인 경우는 NULL이 맨 앞에 옴.
...
/*
    ...
    - GROUP BY에 사용 되지 않은 컬럼 값은 반드시 그룹함수 사용해야함.
*/
-- MBTI 별 평균나이, 나이 합계, 총 인원

SELECT MBTI, ROUND(AVG(AGE)), SUM(AGE), COUNT(*)
FROM USER_INFO
GROUP BY MBTI;
...
SELECT 
  DECODE(SUBSTR(EMP_NO, 8, 1), 1, '남', 2, '여') 성별, COUNT(*) "사원 수" 
FROM EMPLOYEE
...
-- USER_INFO에서 MBTI별 평균 나이를 계산하는데 평균 나이가 30 이하인 MBTI만 조회, 100살 이상인 경우 제외
SELECT MBTI, AVG(AGE)
FROM USER_INFO
WHERE AGE < 100
GROUP BY MBTI
