...
public class B_Aggregate {
	...
	/*
	 * 기본 집계 - 최종 처리 기능으로 요소들을 개수, 합계, 평균, 최대값, 최소값 등과 같이 하나의 값으로 산출하는 역할
	 * 
	 * - count : 요소의 개수 리턴 - sum : 요소의 합계 리턴 - average : 요소들의 평균 리턴 - max : 최대 요소 리턴
	 * - min : 치소 요소 리턴 -findFirst : 첫번째 요소 리턴
	 */
	public void method1() {
		...
		/*
		 * Optional 클래스 - 스트림의 최종 결과 값을 저장하는 객체 - 단순히 값만 저장하는 것이 아니라, 값의 존재 여부 확인하고 값이
...
//					값이 저장되어 있지 않을 경우 디폴트 값 저장
		...
		System.out.println("최대 값 : " + maxResult);
	...
	/*
	 * 커스텀 집계 - 스트림에서 기본 집계 메서드를 제공하지만 다양한 집계 결과물을 만들 수 있도록 reduce 메서드 제공
	 * 
	 */
	public void method2() {
