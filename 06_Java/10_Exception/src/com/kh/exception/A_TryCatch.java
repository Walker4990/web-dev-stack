...
public class A_TryCatch {
	...
	/*
	 * 컴파일 에러(compile-time error) : 컴파일 시에 발생하는 에러 (실행하기 전부터 발생)
	 * 런 타임 에러(runtime error) : 실행 시에 발생하는 에러
	 ...
	 * 
	 * 에러는 메모리 부족(OutOfMemoryError)이나 스택오버플로우(StackOverfloewError)와 같이
	...
	public static void main(String[] args) {
		try {
			...
			// printStackTrace : 예외발생 당시에 호출스택(Call Stack)에 있었던 메서드의 정보와
//									예외 메시지를 화면에 출력
			e.printStackTrace();
