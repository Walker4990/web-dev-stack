package com.kh.inheritane.model;
 ...
 * - 명시하지 않아도 모든 클래스는 Object 클래스 상속
 *   -> Object 클래스의 메서드들을 오버라이딩 하여 메스더 재정의 가능
 * - 부모 클래스의 생성자, 초기화 블록은 상속되지 않는다.
 * 	 -> 자식 클래스 생성 시 부모 클래스 생성자가 먼저 실행
 * - 부모의 private 변수는 상속은 되지만 직접 접근은 불가
...
public class Notebook extends Product {
	...
	public Notebook(String brand, String pCode, String name, int price, String cpu) {
		// super : 해당 객체의 부모 객체 주소를 담고 있음
		// 1. 부모생성자 호출
		super(brand, pCode, name, price);
		this.cpu = cpu;
		
		// 2. super.을 통해서 부모의 변수에 직접 접근
		super.brand = brand; // 부모가 public - 어디서든 접근가능
...
//		super.price = price // 부모가 private - 직접 접근 X
	...
	/*
	 * 오버라이딩(overriding)
	 * - 자식 클래스가 상속받은 부모의 클래스에 메서드를 재정의 하는 것
	 * */
	@Override
